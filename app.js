...
// (7 bar será interpolado proporcionalmente usando base 6 bar)
const data = [
...
// Retorna força para a pressão p (kgf). Se p não existir (ex.: 7), interpola proporcionalmente.
function getForce(row, p, kind){
  if(row.f[p]) return row.f[p][kind];

  // base 6 bar (ou 10 se não existir)
...
function buildTable(rows){
  ...
  rows.forEach(r => {
    ...
    pressures.forEach(p => {
      const ext = getForce(r, p, "e");
      const ret = getForce(r, p, "r");
      tr += `<td data-p="${p}" data-kind="e">${fmt(ext)}</td>`;
      tr += `<td data-p="${p}" data-kind="r">${fmt(ret)}</td>`;
    });
...
function applyFilter(){
  ...
  // Destaca SOMENTE se os dois estiverem selecionados
  if(boreSel && pSel){
