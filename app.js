...
function getForce(row, p, kind){
  ...
  const baseP = row.f[6] ? 6 : 10;
  return row.f[baseP][kind] * (p / baseP);
}

// ===== UI refs (definidos no init) =====
let elTable, elBore, elPressure, elSearch, elClear;
let elForceKind, elCylForce, elLever, elAngle, elUseTable, elCalcTorque, elResult;
...
function buildTable(rows){
  ...
  for(let i=0;i<pressures.length;i++){
    const p = pressures[i];
    h1 += `<th class="group" colspan="2">${p} bar</th>`;
  }
  h1 += `</tr>`;

  let h2 = `<tr>`;
  for(let i=0;i<pressures.length;i++){
    h2 += `<th>Ext</th><th>Ret</th>`;
  }
  h2 += `</tr>`;

  elTable.innerHTML = h1 + h2;
...
function clearHighlights(){
  ...
  for(let i=0;i<trs.length;i++) trs[i].classList.remove("hl-row");

  const tds = elTable.querySelectorAll("td");
  for(let i=0;i<tds.length;i++) tds[i].classList.remove("hl-cell");
}

function applyFilter(){
  const boreSel = (elBore.value || "").trim();
  const pSel = (elPressure.value || "").trim();
  ...
  if(boreSel && pSel){
    const row = elTable.querySelector(`tr[data-bore="${boreSel}"]`);
    if(row){
      row.classList.add("hl-row");

      const extCell = row.querySelector(`td[data-p="${pSel}"][data-kind="e"]`);
      const retCell = row.querySelector(`td[data-p="${pSel}"][data-kind="r"]`);
      if(extCell) extCell.classList.add("hl-cell");
      if(retCell) retCell.classList.add("hl-cell");
    }
  }
}
...
function useTableForce(){
  const boreSel = (elBore.value || "").trim();
  const pSel = (elPressure.value || "").trim();
  const kind = (elForceKind && elForceKind.value) ? elForceKind.value : "e";
  ...
  elCylForce.value = String(val.toFixed(1)); // input padrão com ponto

  setResult(`Força preenchida com <b>${fmt1(val)} kgf</b> (${kind === "e" ? "Extensão" : "Retração"}) em <b>${pSel} bar</b>.`);
}

function calcTorque(){
  const F_kgf = parseNumberPT(elCylForce.value);
  const r_mm  = parseNumberPT(elLever.value);
  const ang   = parseNumberPT(elAngle.value);

  if(!isFinite(F_kgf) || !isFinite(r_mm) || !isFinite(ang)){
  ...
  const T_kgf_mm = F_kgf * r_mm * sinT;
  const T_kgf_m  = T_kgf_mm / 1000;
...
// ===== Init =====
function init(){
  // refs (se algum estiver faltando, a gente não trava o site inteiro)
  elTable    = document.getElementById("forceTable");
  elBore     = document.getElementById("boreSelect");
  elPressure = document.getElementById("pressure");
  elSearch   = document.getElementById("search");
  elClear    = document.getElementById("clear");

  // torque
  elForceKind = document.getElementById("forceKind");
  elCylForce  = document.getElementById("cylinderForce");
  elLever     = document.getElementById("leverLength");
  elAngle     = document.getElementById("angle");
  elUseTable  = document.getElementById("useTableForce");
  elCalcTorque= document.getElementById("calculateTorque");
  elResult    = document.getElementById("torqueResult");

  if(!elTable || !elBore || !elPressure || !elSearch || !elClear){
    // Pelo menos avisa no console
  ...
  // eventos tabela
  elPressure.addEventListener("change", applyFilter);
  elSearch.addEventListener("input", applyFilter);
  elBore.addEventListener("change", applyFilter);
  ...
  // eventos torque (se existir bloco no HTML)
  if(elUseTable) elUseTable.addEventListener("click", useTableForce);
  if(elCalcTorque) elCalcTorque.addEventListener("click", calcTorque);
}

// roda seguro quando DOM estiver pronto
document.addEventListener("DOMContentLoaded", function(){
  try{
    init();
  }catch(err){
    console.error(err);
    const r = document.getElementById("torqueResult");
    if(r) r.innerHTML = "Erro no JavaScript. Abra o Console (F12) para ver detalhes.";
  }
});
